...
- `Minimum runtime` is een runtime slider in seconden. De ondergrens is `300 s`, en die ondergrens wordt ook in de regeling zelf afgedwongen zodat een oudere bewaarde waarde zoals `60` geen korte compressor-runs kan blijven veroorzaken. Een low-flow-fout of watertemperatuur-trip gaat wel voor: dan wordt de compressor gestopt, ook binnen de minimale looptijd.
...
- Daarbovenop geeft OpenQuatt per compressor maximaal zes startopdrachten per voortschrijdend uur vrij, in verwarmen, koelen en warm tapwater. Een zevende start wacht tot de oudste van die zes een uur oud is; doorlopen, moduleren en stoppen blijven altijd mogelijk. Verdwijnt de vraag tijdens het wachten, dan volgt geen start. Een geblokkeerde start staat in de log (`startlimiet 6/uur bereikt`) en bij Duo in `oq_duo_optimizer_reason` als `start_limit_6_per_hour`. De telling zit alleen in RAM en begint na een herstart opnieuw; ze telt OpenQuatt-opdrachten, geen herstarts die de ODU zelf doet. Een defrost-hold zet alleen nog een lopend commando voort en herstart geen compressor die OpenQuatt al had gestopt.
