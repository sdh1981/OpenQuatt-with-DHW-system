...
class Controller {
 public:
  ...
  void clear_fault_latch(uint32_t now_ms) {
    if (state_ == State::FAULT || fault_ != Fault::NONE || fault_latched_) {
      state_ = State::IDLE_CV;
      fault_ = Fault::NONE;
      state_enter_ms_ = 0;
      fault_latched_ = false;
      // Give sensor inputs a short stabilization window after manual clear.
      // This prevents immediate re-latch while HA/source values settle.
      sensor_clear_grace_until_ms_ = now_ms + sensor_clear_grace_ms_;
      reset_cycle_flags_();
    }
  }
 ...
 private:
  ...
  uint32_t sensor_clear_grace_until_ms_ = 0;
  static constexpr uint32_t sensor_clear_grace_ms_ = 15000UL;
  ...
  void evaluate_faults_(const Inputs &in, const Config &cfg) {
    ...
    const bool sensor_clear_grace_active =
        (sensor_clear_grace_until_ms_ != 0) &&
        ((uint32_t)(sensor_clear_grace_until_ms_ - in.now_ms) < 0x80000000UL);
    ...
    if (sensor_clear_grace_active) return;
